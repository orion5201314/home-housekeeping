...
class UpgradeLogic extends BaseLogic
{
    ...
    public static function verify($params)
    {
        ...
        $remoteUrl = self::BASE_URL . "/api/version/verify?domain=".$domain."&product_id=9&type=2&version_id=".$params['id']."&link=".$params['link'];
    ...
    public static function getRemoteVersion($pageNo = null, $pageSize = null)
    {
        ...
        if (empty($pageNo) || empty($pageSize)) {
            $remoteUrl = self::BASE_URL . "/api/version/lists?product_id=9&type=2&page=1";
        } else {
            $remoteUrl = self::BASE_URL . "/api/version/lists?product_id=9&type=2&page_no=$pageNo&page_size=$pageSize&page=1";
        }
        $result = Requests::get($remoteUrl);
        $result = json_decode($result->body, true);
        $result = $result['data'] ?? [];

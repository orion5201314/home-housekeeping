...
class NoticeLogic extends BaseLogic
{
    ...
    public static function checkSms($item)
    {
        ...
        if (empty($item['template_id']) || empty($item['content']) || empty($item['status'])) {
            throw new \Exception('短信通知必填参数：模板ID、短信内容、开启状态');
        }
    }
    ...
    public static function checkOa($item)
    {
        ...
        if (empty($item['template_id']) || empty($item['first']) || empty($item['remark']) || empty($item['tpl']) || empty($item['status'])) {
            throw new \Exception('微信模板消息必填参数：开启状态、模板ID、模板字段first内容、模板字段remrk内容、模板内容');
        }
    }
    ...
    public static function checkMnp($item)
    {
        ...
        if (empty($item['template_id']) || empty($item['tpl']) || empty($item['status'])) {
            throw new \Exception('微信模板消息必填参数：开启状态、模板ID、模板内容');
        }
    }
}
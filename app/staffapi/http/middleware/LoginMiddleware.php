...
namespace app\staffapi\http\middleware;
...
class LoginMiddleware
{
    ...
    public function handle($request, \Closure $next)
    {
        ...
        $staffInfo = (new StaffTokenCache())->getStaffInfo($token);

        if (empty($staffInfo) && !$isNotNeedLogin) {
        ...
        if ($staffInfo) {
            //获取临近过期自动续期时长
            $beExpireDuration = Config::get('project.staff_token.be_expire_duration');

            //token续期
            if (time() > ($staffInfo['expire_time'] - $beExpireDuration)) {

                $result = StaffTokenService::overtimeToken($token);
        ...
        $request->staffId = $staffInfo['staff_id'] ?? 0;

...
class SubscribeLogic extends BaseLogic
{
    ...
    public static function lists($get)
    {
        ...
        if (!empty($get['scene']) && $get['scene'] == 'before_pay') {
            $where[] = ['scene_id', '=', 201];
        }
        if (!empty($get['scene']) && $get['scene'] == 'after_pay') {
            $where[] = ['scene_id', 'in', [202,203,204]];
        }
        if (!empty($get['scene']) && $get['scene'] == 'cancel_order') {
            $where[] = ['scene_id', 'in', [205,206]];
        }

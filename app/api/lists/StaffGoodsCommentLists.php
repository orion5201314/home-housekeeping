...
class StaffGoodsCommentLists extends BaseShopDataLists
{
    /**
     * @notes 搜索条件
     * @return array
     * @author ljj
     * @date 2024/10/24 下午6:12
     */
    public function where()
    {
        $staffOrderIds = Order::where(['staff_id'=>$this->params['staff_id'],'order_status'=>OrderEnum::ORDER_STATUS_FINISH])->column('id');
        $staffOrderGoodsIds = OrderGoods::where(['order_id'=>$staffOrderIds])->column('id');
        $where[] = ['gc.order_goods_id','in',$staffOrderGoodsIds];
